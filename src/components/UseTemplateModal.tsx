...
export default function UseTemplateModal({ prompt, isOpen, onClose }: UseTemplateModalProps) {
  ...
  return (
    <div id="use-template-modal-backdrop" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div id="use-template-modal-container" className="bg-white rounded-2xl shadow-xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden border border-slate-100">
        
        {/* Modal Top Header */}
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <span className="text-[9px] text-purple-600 font-bold uppercase tracking-wider block mb-0.5">
        ...
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-slate-450 gap-3">
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="w-1/2 overflow-y-auto p-6 border-r border-slate-100 space-y-5">
              ...
              {variables.length === 0 ? (
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-[11px] text-slate-450 leading-relaxed">
                  ...
                  {variables.map((variable) => (
                    <div key={variable.id} className="space-y-1">
                      <div className="flex items-center justify-between text-xs font-semibold">
                        <label className="text-slate-700 flex items-center gap-1.5 leading-tight">
                          <span>{variable.label || variable.name}</span>
                          {variable.is_required && (
                            <span className="text-red-500 text-[10px]">*</span>
                          )}
                        </label>
                        <span className="text-[9px] font-mono text-purple-500 bg-purple-50 px-1 rounded">
                          {`{${variable.name}}`}
                        </span>
                      </div>
                      
                      {variable.description && (
                        <div className="text-[10px] text-slate-400 font-medium pb-0.5">
                          {variable.description}
                        </div>
                      )}
                      ...
                      <textarea
                        ...
                        className="w-full text-xs p-3 bg-slate-50 rounded-xl border border-slate-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all font-sans placeholder-slate-400"
                      />
                    </div>
                  ))}
                </div>
            ...
            <div className="w-1/2 overflow-hidden flex flex-col bg-slate-50/50">
              <div className="p-4 border-b border-slate-150 flex items-center justify-between bg-slate-50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-slate-700 text-xs">คำสั่งที่เรียบเรียงสำเร็จ (Compiled Prompt)</span>
                </div>
                
                {prompt.tool_name && (
                  <span className="text-[9px] bg-white border border-slate-200 py-0.5 px-2 rounded-md font-mono text-slate-500 font-bold">
              ...
              <div className="flex-1 overflow-y-auto p-5 font-mono text-xs text-slate-700 space-y-4">
                {blocks.map((block) => {
                  ...
                  return (
                    <div key={block.id} className="p-4 rounded-xl bg-white border border-slate-200 text-left leading-relaxed relative pl-6 group">
                      {/* Left Block Line indicator tag */}
                      <div className="absolute top-0 left-0 bottom-0 w-1.5 rounded-l-xl bg-purple-200 group-hover:bg-purple-500 transition-all"></div>
                      
                      <div className="flex items-center gap-1 text-[10px] font-bold uppercase text-purple-600 tracking-wider mb-2 select-none">
                        <ChevronRight className="w-3.5 h-3.5 shrink-0" />
                        <span>{block.name || block.block_type}</span>
                      </div>
                      
                      <pre className="whitespace-pre-wrap font-mono text-[11px] font-medium leading-relaxed">
                        {renderedTxt}
                      </pre>
                    </div>
                  );
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-white text-xs">
          <div className="flex items-center gap-2 text-slate-400 font-semibold leading-none">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={onClose}
              className="px-4 py-2 text-slate-600 hover:bg-slate-50 active:bg-slate-100 font-semibold rounded-lg transition-all"
            >
              ยกเลิก
            </button>
            <button
              ...
              disabled={loading}
              className={`flex items-center gap-1.5 py-2 px-5 font-bold text-white shadow-md transition-all rounded-xl hover:-translate-y-0.5 active:translate-y-0 ${
                copied 
